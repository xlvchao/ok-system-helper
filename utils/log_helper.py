...
filehandler = logging.handlers.TimedRotatingFileHandler(base_dir+"/logs/ok-system-helper.log", when='D', interval=1, backupCount=31)#每 1(interval) 天(when) 重写1个文件,保留7(backupCount) 个旧文件；when还可以是Y/m/H/M/S
